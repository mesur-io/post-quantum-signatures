...
const signRaw = async (msg, privateKey) =>{
  const signature = await sphincs.sign(msg, privateKey)
  return signature
}

const verifyRaw = async (sig, publicKey) =>{
  const verification = await sphincs.open(sig, publicKey)
  return verification
}
...
const sign = async ({header, payload, privateKeyJwk})=>{
  ...
  const tbs = `${base64url.encode(JSON.stringify({alg: privateKeyJwk.alg, ...header}))}.${base64url.encode(tbs1)}`
